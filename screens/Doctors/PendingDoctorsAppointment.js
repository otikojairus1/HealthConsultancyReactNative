...
let data=[];
export default function PendingDoctorsAppointments({navigation, logged}) {
  ...
  const [isLoading, setIsLoading] = useState(true);
    useEffect(() => {
      data = [];
        ...
        axios.post('https://consultancy-api.herokuapp.com/api/pending/appointments/doctors',{
          "doctorEmail": logged
        })
...
//console.log(data);
  ...
  const renderItem = ({ item, index }) => (
    ...
    <Box>
      <Pressable onPress={
  ()=>{
    nav.navigate('Confirm Appointment', {
      "id": item.id,
        ...
        <Box
          ...
          <HStack alignItems="center" space={3}>
            ...
            <VStack>
              ...
              <Text color="coolGray.600" _dark={{ color: 'warmGray.200' }}>{item.time}</Text>
            </VStack>
            <Spacer />
            <Text color="coolGray.600" _dark={{ color: 'warmGray.200' }}>{item.id}</Text>
