...
const onSubmitHandler = (event) => {
  ...
  axios({
    method: 'post',
    url: 'https://consultancy-api.herokuapp.com/api/doctor/login',
    data: {
      "email":email,
      "password":password
  },
    config: { headers: {'Content-Type': 'application/json' }}
     ...
     .then(function (response){
      ...
      if(typeof response.data.error !== "undefined"){
        ...
        setIsLoading(false);
        navigation.navigate('Doctor Dashboard', {"test":response.data.response.email});
...
if (alert){
  ...
  return (
    ...
    <Box safeArea p="2" py="8" w="90%" maxW="290">
      ...
      <VStack space={3} mt="5">
        <FormControl>
          <FormControl.Label>Doctor's Email ID</FormControl.Label>
